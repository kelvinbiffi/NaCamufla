...
class functions{
	...
	function createChat($chatId,$ownerNick,$ownerCode,$dateExpiration,$howMuchKeep,$ownerLeaveDelChat = "true"){
		...
		if($resp == false){
			...
			$jsonBase = '{"chat":{"code":"'.$chatId.'","ownerLeaveDelChat":'.$ownerLeaveDelChat.',"dateExpiration":"'.$dateExpiration.'","howMuchKeep":"'.$howMuchKeep.'","participants":{"0":{"code":"'.$ownerCode.'","nick":"'.$ownerNick.'","owner":true}},"msg":{}}}';
			$c = fwrite($a, $jsonBase);
			fclose($a);
		}else{
			$c = true;
